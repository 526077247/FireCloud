...
namespace TaoTie
{
    ...
    public class SkillWatcher_Cost : ISkillWatcher
    {
        ...
        public void Run(SkillPara para)
        {
            ...
            {
                Log.Error(para.Ability.Config.Id+"技能配置消耗属性和公式数量不对");
                return;
            }
            ...
            if(NumericType.Map.TryGetValue(idKey,out int attrId))
            {
                ...
                if (formula != null)
                {
                    ...
                    NumericComponent f = para.From.GetComponent<NumericComponent>();
                    NumericComponent t = para.To?.GetComponent<NumericComponent>();
