...
namespace TaoTie
{
    public class GameObjectHolderComponent:Component,IComponent
    {
        ...
        {
            var obj = await GameObjectPoolManager.Instance.GetGameObjectAsync(this.GetParent<Unit>().Config.Perfab);
            if (this.IsDispose)
            {
                GameObjectPoolManager.Instance.RecycleGameObject(obj);
                return;
            ...
            Messager.Instance.AddListener<Unit,Quaternion>(Id,MessageId.ChangePositionEvt,OnChaneRotation);
        }

        public void Destroy()
        {
            Messager.Instance.RemoveListener<Unit,Vector3>(Id,MessageId.ChangePositionEvt,OnChanePosition);
            Messager.Instance.RemoveListener<Unit,Quaternion>(Id,MessageId.ChangePositionEvt,OnChaneRotation);
            foreach ((long k,Collider value) in Triggers)
            {
